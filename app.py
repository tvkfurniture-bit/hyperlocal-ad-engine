...
if generate_btn and biz_name and biz_address:
    ...
    with st.spinner("🧠 AI is architecting Offers, Detailed Storyboards, and Sales Scripts..."):
        
        # 1. GENERATE STRATEGY
        strategy_prompt = f"""
        Business: {biz_name} ({biz_niche}) at {biz_address}. Platform: {platform}. {comp_prompt}
        ...
        Task 3: Write a cold outreach DM and Cold Email to {biz_name} pitching this ROI-focused campaign.
        ...
        # 2. GENERATE HOLLYWOOD-LEVEL VIDEO PROMPTS
        plat_rules = ""
        if platform == "TikTok / IG Reels":
            plat_rules = "Fast camera movements (whip pans, FPV drone, handheld POV), UGC feel but high aesthetic quality."
        elif platform == "YouTube Pre-Roll":
            plat_rules = "High-budget cinematic feel, smooth gimbals, dramatic lighting, high tension."
        else:
            plat_rules = "Corporate aesthetic, clean bright lighting, premium Sony/RED camera feel, slow pans."

        vid_prompt = f"""
        You are a Technical Prompt Engineer for Runway Gen-3 and Luma Dream Machine.
        ...
        1. DO NOT write short prompts. Every visual prompt MUST be 40-60 words long.
        2. NO TEXT in the video generation. Do not ask the AI to render signs, logos, or words.
        3. Use this strict formula for every visual prompt: 
           [Camera Movement] + [Extremely detailed subject/action] + [Lighting & Environment] + [Camera Gear: e.g., ARRI Alexa, 35mm lens, f/1.8, 4k, photorealistic].
        4. Do not repeat angles. Scene 1 wide, Scene 2 macro, Scene 3 motion, etc.

        Format EXACTLY like this for EACH of the {num_scenes} scenes:
        ### SCENE [X]
        🎥 **Visual Prompt:** [Insert your 40-60 word highly technical, photorealistic prompt here]
        🗣️ **VoiceOver Script:** [1 snappy, natural sentence matching the {platform} tone]
        ✍️ **Manual Text Overlay:** [2-5 punchy marketing words to type manually later]
        """
        vid_response = client.chat.completions.create(
            model="llama-3.1-8b-instant",
            messages=[{"role": "system", "content": "You are a Master Prompt Engineer for AI Video Generation."}, {"role": "user", "content": vid_prompt}]
        )
...
if 'strategy' in st.session_state:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🎁 Mafia Offers & Copy", "🎬 The Detailed Storyboard", "💬 Client Outreach Scripts", "📄 Export Pitch Deck"])
    ...
    with tab2:
        ...
        st.info("💡 **PRO TIP:** Copy the dense `Visual Prompt` directly into Runway Gen-3 or Luma Dream Machine. The technical jargon ensures photorealism.")
